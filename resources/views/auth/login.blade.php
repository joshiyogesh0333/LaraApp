...
@media (max-width: 991px){
  h2.brand-text.text-primary.ml-1
  {
    display: none;
  }
...
<div class="auth-wrapper auth-v2">
  <div class="auth-inner row m-0">
      ...
      <a class="brand-logo" href="javascript:void(0);">
        ...
        <h2 class="brand-text text-primary ml-1">Anu Construction</h2>
      </a>
      <!-- /Brand logo-->
      <!-- Left Text-->
      ...
      <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
        <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
          ...
          <img id="logo" src="{{url('/public/images/logo/large-logo.png')}}">
            </div>
          <h2 class="card-title font-weight-bold mb-1">Welcome to Anu Construction!</h2>
          <!-- <p class="card-text mb-2">Please sign-in to your account and start the adventure</p> -->
          <form method="POST" action="{{ route('login') }}">
            @csrf
            <div class="form-group">
              <label class="form-label" for="login-email">Email / User ID</label>
              <input class="form-control @error('email') is-invalid @enderror " id="login-email" type="text" name="email" value="{{ old('email') }}"   aria-describedby="login-email" autofocus="" tabindex="1" required autocomplete="email" autofocus />
              @error('email')
